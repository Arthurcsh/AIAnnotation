...
import com.jdc.diffverificate.service.ConfigUtil;
...
public class HttpClientUtil {
    public static final String LOGIN_URL = "https://eistest.unishining.com/prompt-dev-api";
    ...
    public static <T> T postForObjectWithToken(String url, String reqBody, Map<String, String> headers, Class<T> resType) throws Exception {
        String oauthToken = ConfigUtil.getOauthToken();
        if (headers == null) {
            headers = Maps.newHashMap();
        }
        headers.put("Cookie", "token=" + oauthToken);
    ...
    public static void loginAndSaveToken() throws Exception {
        Pair<String, String> pair = ConfigUtil.getKUser();
        String kUsername = pair.getFirst();
        if (StringUtils.isBlank(kUsername)) {
            throw new RuntimeException("请先在更新配置菜单配置Kuser用户信息");
        }
        String kPassword = pair.getSecond();
        String accessToken = loginByUrl(kUsername, kPassword);
        ConfigUtil.saveOauthToken(accessToken);
    }

    public static String loginByUrl(String kUsername, String kPassword) throws Exception {
        String reqBody = String.format("grant_type=password&username=%s&password=%s", kUsername, kPassword);
        Map<String, String> headers = Maps.newHashMap();
        headers.put("Content-Type", "application/x-www-form-urlencoded");
        JsonObject jsonObject = HttpClientUtil.postForObject(LOGIN_URL, reqBody, headers, JsonObject.class);
        String accessToken = jsonObject.get("access_token").getAsString();
//        NotifyUtil.notifyInfo(ProjectUtil.getActiveProject(), "请求url:" + LOGIN_URL + "," + kUsername + "登录结果:" + jsonObject);
        return accessToken;
    }
}
